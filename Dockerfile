...
RUN cd / && wget https://www.atlassian.com/software/jira/downloads/binary/atlassian-jira-6.0-x64.bin \
    && sha256sum -c check.gpr \
    && chmod a+x atlassian-jira-6.0-x64.bin \
    && cd / && ./atlassian-jira-6.0-x64.bin < ./installjira \
    && rm /atlassian-jira-6.0-x64.bin \
